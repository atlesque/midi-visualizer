...
export function drawMidiVisualization(
  ...
  const trackHeight = tracks.length > 0
    ? Math.max(40, (height - 40) / tracks.length)
    : height

  // Background grid
  drawGrid(ctx, width, height, meta.bpm, meta.timeSignatureUpper, scrollOffset, timeRange)
...
function drawTrackNotes(
  ...
  for (const note of track.notes) {
    const x = xStart + ((note.time - scrollOffset) / timeRange) * (ctx.canvas.width / devicePixelRatio - xStart)
    const w = Math.max(1, (note.duration / timeRange) * (ctx.canvas.width / devicePixelRatio - xStart))
